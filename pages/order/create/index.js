...
Page({
  ...
  dateDiff: function (d1, d2) {
    ...
    onLoad: function(t) {
          // check_in_time: t.check_in_time,
              ...
              //   option_stock_id: t.option_stock_id,
              //   stock_time: t.stock_time,
      console.log(t)
      if (t.idd==1){
        this.setData({
          option_stock_id:t.option_stock_id,
          stock_time: t.stock_time,
          'goodslist[0]option_stock_id': t.option_stock_id,
          'goodslist[0]stock_time': t.stock_time,
           idd: t.idd
        })
      }else{
        this.setData({
          option_stock_id: '',
          stock_time: '',
          'goodslist[0]option_stock_id': '',
          'goodslist[0]stock_time': '',
          idd:10
        })
      }
      if ( t.idd == 2) {
        this.setData({
          'goodslist[0]check_out_time': t.check_out_time,
          check_in_time: t.check_in_time,
          check_out_time: t.check_out_time,
          'goodslist[0]check_in_time': t.check_in_time,
          idd: t.idd
        })
        this.dateDiff(t.check_out_time,t.check_in_time)
      } else {
        this.setData({
          check_in_time: '',
          check_out_time:'',
          'goodslist[0]check_out_time': '',
          'goodslist[0]check_in_time': '',
          idd:10
        })
      }
  ...
  nametit:function(e){
    ...
    console.log(this.data.name_tit)
  },
  // 手机号
  phonetit: function (e) {
    this.setData({
    ...
    console.log(this.data.phone_tit)
  },
  // 身份证
  idtit: function (e) {
    this.setData({
      id_tit: e.detail.value
    })
    console.log(this.data.id_tit)
  },
  //护照
  passporttit: function (e) {
    this.setData({
      passport_tit: e.detail.value
    })
    console.log(this.data.passport_tit)
  },
  // 预算
  budgettit: function (e) {
    this.setData({
      budget_tit: e.detail.value
    })
    console.log(this.data.budget_tit)
  },
  mantit: function (e) {
    this.setData({
      man_tit: e.detail.value
    })
    console.log(this.data.man_tit)
  },
    ...
    number: function(t) {
      ...
      console.log(d)
        s.each(c.goods, function(t, e) {
          ...
          console.log(n),
         c.goodsprice = s.toFixed(l, 2), e.setData({
            list: c,
  ...
  inputbind:function(e){
    ...
    var that = this
    that.setData({
      'list.goods[0].goods[0].total': e.detail.value
    })
    var c = this.data.list
    var l = parseFloat(e.detail.value * c.goods[0].goods[0].price);
    console.log(l)
    c.goodsprice = s.toFixed(l, 2), that.setData({
      list: c,
      goodslist: that.getGoodsList(c.goods)
    }),
    this.caculate(c);
    // setTimeout(function(){
    ...
    submit: function() {
        ...
        console.log(t.list)
        if (t.list.goods[0].goods[0].is_budget==2){
          if (!t.budget_tit){
            wx.showToast({
              title: '完善预算信息',
              icon:'none'
      ...
      if (t.list.goods[0].goods[0].is_fire_time == 2) {
        if (!t.ops) {
          wx.showToast({
            title: '完善出发时间',
            icon: 'none'
      ...
      if (t.list.goods[0].goods[0].is_id_num == 2) {
        if (!t.id_tit) {
          wx.showToast({
            title: '完善身份证',
            icon: 'none'
      ...
      if (t.list.goods[0].goods[0].is_passport == 2) {
        if (!t.id_tit) {
          wx.showToast({
            title: '完善护照',
            icon: 'none'
      ...
      if (t.list.goods[0].goods[0].is_name == 2) {
        if (!t.name_tit) {
          wx.showToast({
            title: '完善姓名',
            icon: 'none'
      ...
      if (t.list.goods[0].goods[0].is_time == 2) {
        if (!t.op) {
          wx.showToast({
            title: '完善时间',
            icon: 'none'
      ...
      if (t.list.goods[0].goods[0].is_moblie == 2) {
        if (!t.phone_tit) {
          wx.showToast({
            title: '完善电话号码',
            icon: 'none'
      ...
      if (t.list.goods[0].goods[0].is_men_num == 2) {
        if (!t.man_tit) {
          wx.showToast({
            title: '完善人数',
            icon: 'none'
        ...
        !t.submit && r.verify(this, i)) {
            ...
            var o = {
              ...
              //   stock_time: t.stock_time,
                ...
                goods: t.goodslist,
                order_name: t.name_tit,
                order_moblie: t.phone_tit,
                order_time: t.op,
                order_fire_time: t.ops,
                budget:t.budget_tit,
                men_num: t.man_tit,
                id_num: t.id_tit,
                passport_num:t.passport_tit,
                gdid: t.options.gdid,
